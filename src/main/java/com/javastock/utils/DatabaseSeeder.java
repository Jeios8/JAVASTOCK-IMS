...
public class DatabaseSeeder {
    ...
    private void seedInventory() {
        ...
        for (int i = 0; i < inventory.length; i++) {
            // Extract product_id and warehouse_id from your existing inventory array
            int productId = (int) inventory[i][0];
            int warehouseId = (int) inventory[i][1];

            // Retrieve reorder level from your 'products' array
            // productId - 1 to account for zero-based index in the array
            int reorderLevel = (int) products[productId - 1][5];

            // Randomize quantity: allow it to be anywhere from 0 up to ~3× reorder level
            int quantity = random.nextInt(reorderLevel * 3 + 1);

            // Randomize expiration date (3 to 12 months ahead)
            String expirationDate = today.plusMonths(random.nextInt(10) + 3).toString();

            updatedInventory[i][0] = productId; // product_id
            updatedInventory[i][1] = warehouseId; // warehouse_id
            updatedInventory[i][2] = quantity; // quantity
